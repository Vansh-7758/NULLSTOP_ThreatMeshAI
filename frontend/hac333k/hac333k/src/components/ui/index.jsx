...
/**
 * Button — premium CTA button with glow variants
 * @param {'primary'|'ghost'|'outline'|'glass'} variant
 */
export function Button({ children, variant = 'primary', className = '', onClick, href, ...props }) {
  const base = `
...
/**
 * SectionHeading — consistent section label + title + subtitle
 */
export function SectionHeading({ eyebrow, title, subtitle, align = 'center', titleClass = '' }) {
...
/**
 * GlowDot — pulsing location dot for threat map
 */
export function GlowDot({ color = '#5F01FB', size = 10, className = '' }) {
  return (
...
/**
 * AnimatedCounter — scroll-triggered count-up
 */
